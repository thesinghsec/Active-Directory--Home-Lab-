...
- As the command we inserted to our terminal results in enabling the remote desktop on WorkStation-01.
...
- We can disable the same by using the Covenant cmd `powershell reg add “HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server” /v fDenyTSConnections /t REG_DWORD /d 0 /f; Disable-NetFirewallRule -DisplayGroup "Remote Desktop"`

...
- As we have high integrity user session on Covenant we can do persistence by adding a new user in local admin group.
...
- We need to run the uploaded binary, this let's explore the `Task` tab in Grunts and search for PersistAutorun and paste the Targeted path.
...
- To verify the new insatalled registery we can run the cmd: `GetRegistryKey HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run`
...
## Session Passing to Metasploit, SOCKS, and Autoroute Module
...
```
- Now, run `autoroute` cmd against the IP by using command:
...
```
- We have the routing table now we have to do port-forwording using meterpreter cmd `portfwd`:
...
```
In order to forword the port to Covenant we will utilise `socks_proxy`.
...
```
- Here we got port 1234 is set to listening successfully.
- So, now head towards the Covenant and setuo new listener by inserting `BindPort`, `ConnectPort`, and `ConnectAddress` correctly.
...
- Now, go back to `launchers` > select `PowerShell launcher` > select new generated listener and hit Generate.

![image](https://github.com/thesinghsec/Active-Directory--Home-Lab-/assets/126919241/b242b855-63ed-40ed-a840-80bccca95fa3)

- Move to `Host` tab and host a random file example `rev.ps1`.
- Copy the encoded powershell command and run in `WorkStation-02`
